import React, { createContext, useState, useEffect, useContext, useMemo, forwardRef } from 'react';
import { z } from 'zod';
import { Object3D, Euler, Vector3, MathUtils, PerspectiveCamera } from 'three';
import { v4 } from 'uuid';
    ...
    _handlePacket(event, packet) {
        switch (event) {
            ...
            case PacketEvent.Chat: {
                const parsed = z
                    .object({
                    m: z.string().min(1).max(CHAT_MAX_MESSAGE_SIZE),
                })
                    .parse(packet);
...
const useControllerProp = (controller, name) => {
    const [prop, setProp] = useState(() => controller.data[name]);
    useEffect(() => {
...
export { Box, EngineProvider, Gltf, Group, Line, Model, useCamera, useChat, useCommand, useControllerProp, useCursorLock, useEngine, useEvent, useFrame, useGameKey, useInterfaces, useKey, useNetwork, useObject, useObjects, useOnTicks, usePlayer, usePlayerId, usePlayers, useStreamedObject, useStreamedPlayer, useUI };
